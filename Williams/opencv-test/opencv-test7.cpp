...
#else
    cv::String path("/home/why/images/*.png"); //select only jpg
    vector<cv::String> fn;
    vector<cv::Mat> data;
...
#ifndef NDEBUG
        Mat bgr[3];
        split(pDarkImage, bgr);
//        imshow("red", bgr[2]);
        imshow("blue", bgr[0]);
        imshow("difference", bgr[0] - bgr[2]);
...
//        cvtColor(pDarkImage, pGrayImage, COLOR_RGB2GRAY);
        blur(bgr[0] - bgr[2], pMarginImage, Size(5, 5));//use blur to reduce noise
        Canny(pMarginImage, pMarginImage, 100, 300);

        /// Margin detection and ellipse fitting
        vector<Vec4i> hierarchy;
        std::vector<std::vector<Point>> contours;
        /// Find contours
        findContours(pMarginImage, contours, hierarchy, CV_RETR_EXTERNAL, CHAIN_APPROX_NONE, Point(0, 0));
...
//                if ((nTargetColor == TARGET_RED && red < blue) || (nTargetColor == TARGET_BLUE && blue < red)) continue;
                ...
                if ((tmp.angle < 30 || tmp.angle > 150) && tmp.size.width > 3 && heightToWidth > 1.5 &&                    heightToWidth < 9) {
