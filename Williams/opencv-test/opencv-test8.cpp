...
int main(int argc, char **argv) {
    ...
    cv::String path("/home/why/images/red/*.png"); //select only jpg
    vector<cv::String> fn;
    vector<cv::Mat> data;
    cv::glob(path, fn); // recurse

    int num_files = fn.size();
    pSrcImage = imread(fn[0]);
    int cur_file = 0;
//    VideoCapture cap;
//    if (argc == 2) {
//        cap.open(argv[1]);
...
//    printf("exposure = %.2f\n", cap.get(CV_CAP_PROP_EXPOSURE));
    ...
    for (int i = 0; i < 256; ++i)
        p[i] = saturate_cast<uchar>(pow(i / 255.0, 15) * 255.0);

    for (int tenFrame; cur_file <= num_files; tenFrame++) {
        armors.clear();
        frameCount++;
...
//        inRange(pHSV, Scalar(0, 100, 200), Scalar(179, 200, 255), pBinaryBrightness);
        if (nTargetColor == TARGET_RED) {
            Mat pBinaryColorLower, pBinaryColorUpper;
            inRange(pHSV, Scalar(0, 100, 200), Scalar(30, 255, 255), pBinaryColorLower);
            inRange(pHSV, Scalar(170, 100, 200), Scalar(179, 255, 255), pBinaryColorUpper);
            pBinaryColor = pBinaryColorLower | pBinaryColorUpper;
        } else {
            inRange(pHSV, Scalar(85, 100, 200), Scalar(125, 255, 255), pBinaryColor);
        }

        pBinaryBrightness = pBinaryColor;
...
//        unsigned int colorPointSize = colorPoint.size();
        ...
        for (int i = 0; i < count - 1; i++) {
            for (int j = i + 1; j < count; j++) {
                ...
                float upperY = min(e1.center.y + e1.size.height / 2., e2.center.y + e2.size.height / 2.);
                float lowerY = max(e1.center.y - e1.size.height / 2., e2.center.y - e2.size.height / 2.);
                float upperX = max(e1.center.x - e1.size.width / 2., e2.center.x - e2.size.width / 2.);
                float lowerX = min(e1.center.x + e1.size.width / 2., e2.center.x + e2.size.width / 2.);
                upperX = upperX >= pSize.width ? pSize.width - 1 : upperX < 0 ? 0 : upperX;
                lowerX = lowerX >= pSize.width ? pSize.width - 1 : lowerX < 0 ? 0 : lowerX;
                upperY = upperY >= pSize.height ? pSize.height - 1 : upperY < 0 ? 0 : upperY;
                lowerY = lowerY >= pSize.height ? pSize.height - 1 : lowerY < 0 ? 0 : lowerY;

                float average_between = mean(Mat(bgr[1], Rect(Point(lowerX, upperY), Point(upperX, lowerY))))[0];

                if ((angleDifference < 10 || angleDifference > 170) && heightDifferenceRatio < 0.2 &&
                    xDifferenceRatio > 0.5 &&
                    xDifferenceRatio < 3 && yDifferenceRatio < 0.3 && widthDifferenceRatio < 0.6 && average_between < 125) {
//                    cout << average_between << endl;
//                    imshow("debug", Mat(bgr[1], Rect(Point(lowerX, upperY), Point(upperX, lowerY))));
//                    waitKey(0);
                    Armor armor;
