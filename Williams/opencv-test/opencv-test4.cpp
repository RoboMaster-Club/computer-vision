...
int main(int argc, char **argv) {
    ...
    int nTargetColor = 2;
    Mat pSrcImage, pDstImage, pGrayImage, pDarkImage, pMarginImage, pResultImage, pContourEllipse, pContour, pBinaryColor;
#ifdef NDEBUG
    Mat pHSV;
#endif
    ...
    while (pSrcImage.data) {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                ...
                if (hsv[1] >= 0 && hsv[1] <= 100 && hsv[2] >= 200 && hsv[2] <= 255) {
...
//        cvtColor(pDarkImage, pGrayImage, COLOR_RGB2GRAY);
        blur(pBinaryColor, pMarginImage, Size(3, 3));//use blur to reduce noise
